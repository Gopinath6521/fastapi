...
@router.post("/")
def azure_stop_vm(request: AzureStopVM):
    ...
    for vm, rg in zip(vms, rgs):
        ...
        try:
            print(f"Stopping VM: {vm} in {rg}...")
            update_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"Stopping VM: {vm} in {rg}...")
            async_stop = compute_client.virtual_machines.begin_power_off(
                resource_group_name=rg,
                vm_name=vm
            )
            async_stop.result()
            print(f"✅ {vm} stopped successfully!")
            update_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"✅ {vm} stopped successfully!")
            success_vms.append({"vm": vm, "resource_group": rg})
        except Exception as e:
            print(f"❌ Error stopping {vm} in {rg}: {e}")
            update_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"❌ Error stopping {vm} in {rg}: {e}")
            failed_vms.append({"vm": vm, "resource_group": rg, "error": str(e)})

    if len(success_vms) == len(vms):
        print("All the requested servers are stopped successfully")
        close_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"All the requested servers are stopped successfully. VM->{vms}", 3)
    elif len(failed_vms) == len(vms):
        print("All the requested vms failed to stop")
        route_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"All the requested vms failed to stop. Failed VM's->{failed_vms}", 2, "Windows")
    elif len(success_vms) != 0 and len(failed_vms) != 0:
        print(f"Some of the VM's are stopped successfully {success_vms}")
        print(f"Some of the VM's failed to stop {failed_vms}")
        route_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"Failed VM's->{failed_vms} Success VM's->{success_vms}", 2, "Windows")
    else:
        print("Some issue occured to stop VM")
        route_snow_ticket(snow_url, snow_user, snow_pass, sys_id, f"Some issue occured to stop VM", 2, "Windows")
